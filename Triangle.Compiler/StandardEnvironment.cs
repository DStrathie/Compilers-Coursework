...
namespace Triangle.Compiler
{
    public static class StandardEnvironment
    {
        ...
        static TypeDeclaration DeclareStdType(string id, TypeDenoter typedenoter)
            => new TypeDeclaration(new Identifier(id), typedenoter);

        static ConstDeclaration DeclareStdConst(string id, TypeDenoter constType) =>
            new ConstDeclaration(new Identifier(id), new EmptyExpression { Type = constType });
        ...
        static BinaryOperatorDeclaration DeclareStdBinaryOp(string op, TypeDenoter arg1Type, TypeDenoter arg2Type, TypeDenoter resultType)
            => new BinaryOperatorDeclaration(new Operator(op, SourcePosition.Empty), arg1Type, arg2Type, resultType);
    }
}